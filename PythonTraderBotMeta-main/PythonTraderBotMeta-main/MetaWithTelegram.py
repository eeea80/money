...
class Meta:
    ...
    def SendOrder(symbol, lot, buy, sell, ticket=None,pct_tp=0.02, pct_sl=0.01, comment="No specific comment", magic=0):    
        ...
        if buy and ticket==None:
            ...
            try:
                ...
                if magic == 5 or magic == 1000 or magic==4:
                    request['tp']=tp
    ...
    def WaitUntilMarketOpen(symbol, lot, buy, sell, ticket=None, pct_tp=0.02, pct_sl=0.01, magic=0, result=None):
            ...
            if ticket != None:
                ...
                if resume.shape[0] > 0:
                    try:
                        ...
                        exceptMessage = f"An exception has occurred in Meta.run close buy position: {str(e)}"
                        print(exceptMessage)
                        if Meta.teleBotMessage:
                            TeleBot().SendMessage(exceptMessage)
                else:
        ...
        if buy == True or sell ==True:
            ...
            # اگر سفارش خرید داشتی و الان باید ببندیش                
                ...
                if result.comment == "Market closed":
                    print("Market closed!")
                    marketClosed = True
                    result, passBecauseStopLossHit, before = Meta.WaitUntilMarketOpen(symbol, lot, True, False, ticket, pct_tp, pct_sl, magic, result)                
                ...
                if result.comment == "Market closed":
                    print("Market closed!")
                    marketClosed = True
                    result, passBecauseStopLossHit, before = Meta.WaitUntilMarketOpen(symbol, lot, False, True, ticket, pct_tp, pct_sl, magic, result)                
                if (marketClosed and passBecauseStopLossHit == False):
                    print("Market open!")
                    marketClosed = False
                if passBecauseStopLossHit:
                ...
                if result.comment == "Market closed":
                    ...
                    result, passBecauseStopLossHit, before = Meta.WaitUntilMarketOpen(symbol, lot, True, False, None, pct_tp, pct_sl, magic, result)                
                ...
                if result.comment == "Market closed":
                    ...
                    # در سفارش گذاری نیاز به دو متغیر بعد از ریزالت نیست
                    # فقط چون نخواستم دوباره تابع تعریف کنم گذاشتم بمونه
                    result, passBecauseStopLossHit, before = Meta.WaitUntilMarketOpen(symbol, lot, False, True, None, pct_tp, pct_sl, magic, result)                
