...
def SMA(symbol, SMA_Fast_Length=20, SMA_Slow_Length=200):  
    ...
    # ده مقاومت آخر
    resistanceList = resistanceList[0][-10:]

    buySignal1 = df["SMA fast"].iloc[-2] > df["SMA slow"].iloc[-2]
    buySignal2 = df["SMA fast"].iloc[-3] < df["SMA slow"].iloc[-3]
    # روی سطوح مقاومت نباید پوزیشن خرید گرفت
    # You should not take a long position at resistance levels.
    buySignal3 = True if round(df['open'].iloc[-1],-3) not in (resistanceList) else False

    sellSignal1 = df["SMA fast"].iloc[-2] < df["SMA slow"].iloc[-2]
    sellSignal2 = df["SMA fast"].iloc[-3] > df["SMA slow"].iloc[-3]
    # روی سطوح حمایت نباید پوزیشن فروش گرفت
    # You should not take a short position at support levels.
    sellSignal3 = True if round(df['open'].iloc[-1],-3) not in (supportList) else False

    # پیدا کردن نقطه تقاطع دو میانگین تند و کند
    # find the MA fast and slow cross
    buy = buySignal1 and buySignal2 and buySignal3
    sell = sellSignal1 and sellSignal2 and sellSignal3
